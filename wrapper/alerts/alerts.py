...
class NPS_Alerts(object):
    ...
    def alerts_category(api_url):
        ...
        if write.WRITE_TO_FILE == True:
            with open("alert_cats.json", "w") as outfile:
        ...
        return alert_dump
    alerts_category("https://developer.nps.gov/api/v0/alerts")

    def alerts_description(api_url):
        payload = {
        'description':'true'
        }
        alert_url = requests.get(
        ...
        write = NPS_Alerts_Write_Options()
        if write.WRITE_TO_FILE == True:
            with open("alert_description.json", "w") as outfile:
                alert_dump = json.dump(
                alert_url.json(),
                outfile,
                sort_keys = True,
        ...
        alert_dump = alert_url.json()
        return alert_dump
    alerts_description("https://developer.nps.gov/api/v0/alerts")
